...
//* 2번 함수
//* 태그를 제작하는 과정에서 위 1번 함수의 결정된 값을 활용한다.
function makeElement(tagName, sizeValue) {
  //* tagName 매개변수는 반드시 문자열이어야 한다.
  //* sizeValue 매개변수는 반드시 숫자여야 한다.
  //* 태그요소를 생성하고, style 값 중,
  //* with 값과 height 값을 매개변수 sizeValue + px형태로 활용한다.
  let makeElement;
  //* 실행구문
  return makeElement;
}
...
function appendParent(parentElemnt, makeItemElement) {
  //* 위 매개변수 두개는 데이터 타입이 객체여야만 한다.
  parentElemnt.appendchilde(makeItemElement);
}
